...
def split_generated_output(text, expected_count):
    """
    Attempts to parse the generated output text into individual QA pairs.
    
    The output is expected to be a JSON array where each element is an object with keys:
    "question", "answer", and "explanation".
    
    This function first tries to parse the text as JSON. If JSON parsing fails
    (for example, if the LLM produced extra commentary), it falls back to using a regex
    approach to extract QA pairs formatted as:
      - "Question:" followed by text,
      - "Answer:" followed by text,
      - "Explanation:" followed by text.
    
    Returns a list of dictionaries, each with keys "question", "answer", and "explanation".
    """
    try:
        ...
        # JSON parsing failed; proceed to regex fallback.
        pass
...
def get_validated_completion(api_base, model_name, messages, expected_count, max_tokens=2048, temperature=0.7, max_retries=5):
    """
...
def process_record(record, q_type, api_base, model_name, dataset_type):
    ...
    try:
        ...
        qa_result = get_validated_completion(
            ...
            expected_count=question_count, max_tokens=2048, temperature=0.7
        )
